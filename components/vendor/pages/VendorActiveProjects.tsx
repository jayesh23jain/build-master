...
export default function VendorActiveProjects({ showToast }: VendorActiveProjectsProps) {
        ...
        >
          ...
          <div className="px-6 py-6">
            ...
            <div className="grid grid-cols-4 gap-px bg-[#1e2a3a] border border-[#1e2a3a] mb-6">
              {mkStat('Contract Value', project.value, '#34d399')}
              {mkStat('Progress', project.pct + '%', '#a855f7')}
              {mkStat('Deadline', project.deadline, '#f59e4a')}
              {mkStat('Client', project.client, '#e2eef5')}
            </div>
            ...
            <div className="flex justify-between mb-6">
              <div className="font-['JetBrains_Mono'] text-xs text-[#7a9aaa]">{project.pct}% complete</div>
              <div className="font-['JetBrains_Mono'] text-xs text-[#f59e4a]">Due: {project.deadline}</div>
            </div>
