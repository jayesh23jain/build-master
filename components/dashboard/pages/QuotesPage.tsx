...
export default function QuotesPage({ onModal, project }:{ onModal:()=>void; project: any }) {
  const displayQuotes = project?.requests?.map((req: any) => ({
    phase: req.phase?.slice(0, 2) || '01',
    title: req.title || req.phase,
    status: req.quotes?.length > 0 ? 'review' : 'new',
    vendors: req.quotes?.map((q: any) => 
      q.vendor?.user?.firstName ? `${q.vendor.user.firstName} ${q.vendor.user.lastName}` : (q.vendor?.trade || 'Vendor')
    ) || [],
    amounts: req.quotes?.map((q: any) => `₹${(q.amount / 100000).toFixed(1)}L`) || []
  })) || PROJECT.quotes;

  const stats = {
    total: displayQuotes.reduce((acc: number, q: any) => acc + q.vendors.length, 0),
    review: displayQuotes.filter((q: any) => q.status === 'review').length,
    accepted: project?.quotes?.filter((q: any) => q.status === 'ACCEPTED' || q.status === 'accepted').length || 0
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-[1px] bg-[var(--bdr)] border border-[var(--bdr)] mb-2">
        {[['Total Quotes', stats.total, 'var(--cyan)'], ['Pending Review', stats.review, 'var(--amber)'], ['Accepted', stats.accepted, 'var(--green)']].map(([l, v, c]) => (
          <div key={l as string} className="kpi-card">
            <div className="font-['JetBrains_Mono',monospace] text-[.48rem] uppercase tracking-[.14em] text-[var(--muted)] mb-3 flex items-center gap-2">
              <div className="w-[5px] h-[5px] rounded-full pulse-dot" style={{ background: c as string }} />
              {l}
            </div>
          ...
          <div className="px-6 py-4">
            {(q.vendors || []).length > 0 ? (
              <div className="flex flex-col gap-2">
                {q.vendors.map((vendor: string, vi: number) => (
                  <div key={vi} className="q-row">
                    ...
                    <div className="flex-1">
                      <div className="font-['Syne',sans-serif] text-[.82rem] font-[600] text-[var(--text)]">{vendor}</div>
                      <div className="font-['JetBrains_Mono',monospace] text-[.45rem] text-[var(--muted)] mt-[2px]">SUBMITTED 2D AGO</div>
                    </div>
